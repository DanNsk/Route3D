    ...
    public class PropertyBag<TValue> : Dictionary<string, TValue>, IXmlSerializable
    {
        ...
        public void WriteXml(System.Xml.XmlWriter writer)
        {
            foreach (string key in this.Keys)
            {
                ...
                string xmlType = GeneralHelpers.MapTypeToXmlType(type);
                bool isCustom = false;
            ...
            while (reader.Read())
            {
                if (reader.NodeType == XmlNodeType.Element && reader.Name == "key")
                {
                    ...
                    else if (xmlType.StartsWith("___"))
                    {
                        ...
                        Type type = GeneralHelpers.GetTypeFromName(xmlType.Substring(3));
                        XmlSerializer ser = new XmlSerializer(type);
                        value = (TValue)ser.Deserialize(reader);
                    }
                    else
                        value = (TValue)reader.ReadElementContentAs(GeneralHelpers.MapXmlTypeToType(xmlType), null);
        ...
        {
            string xml = null;
            SerializationHelpers.SerializeObject(this, out xml);
            return xml;
        }
        ...
        public bool FromXml(string xml)
        {
            ...
            var result = SerializationHelpers.DeSerializeObject(xml,
